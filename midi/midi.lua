---Polyfill x & 0x7F
local function pf_and_7F(x)
  return x % 128
end

---Polyfill x & 0x7F
local function pf_and_F0(x)
  return math.floor(x / 16) * 16
end

---Polyfill x & 0x7F
local function pf_and_0F(x)
  return x % 16
end

---Polyfill x << 7
local function pf_shl_7(x)
  return x * 128
end

---Polyfill x << n
local function pf_shl_n(x, n)
  return x * (2^n)
end
...
local function readVLQ(stream)
  ...
  repeat
    ...
    -- value = value << 7
    value = pf_shl_7(value)
    -- value = value | byte & 0x7F
    value = value + pf_and_7F(byte)
    length = length + 1
  until byte < 0x80
  return value, length
end
...
local midiEvent = {
  ...
  [0xE0] = function(stream, callback, channel, fb)
    ...
    callback("pitch", channel, (lsb + pf_shl_7(msb)) / 0x2000 - 1)
    return 2
  end
}
...
local metaEvents = {
  ...
  [0x58] = function(data, callback)
    ...
    callback("timeSignature", numerator, pf_shl_n(1, denominator), metronome, dotted)
  end,
...
local function processEvent(stream, callback, runningStatus)
  ...
  if status >= 0x80 and status < 0xF0 then
    ...
    length = length + midiEvent[pf_and_F0(status)](stream, callback, pf_and_0F(status) + 1, firstByte)
